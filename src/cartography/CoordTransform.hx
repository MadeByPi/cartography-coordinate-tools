...
typedef EllipseParameter = {
	var a:Float;
	var b:Float;
	var f:Float;
}

typedef EllipseParameters = {
	var WGS84:EllipseParameter;
	var GRS80:EllipseParameter;
	var Airy1830:EllipseParameter;
	var AiryModified:EllipseParameter;
	var Intl1924:EllipseParameter;
}


typedef DatumTransforms = {
	var toOSGB36:DatumTransform;
	var toED50:DatumTransform;
	var toIrl1975:DatumTransform;
}

typedef DatumTransform = {
...
class CoordTransform {
	...
	// ellipse parameters
	public static var ellipse:EllipseParameters = {
	  WGS84:        { a: 6378137.0,   b: 6356752.3142,   f: 1./298.257223563 },
	  GRS80:        { a: 6378137.0,   b: 6356752.314140, f: 1./298.257222101 },
	...
	public static var datumTransform:DatumTransforms = {
	 ...
	 */
	public static function convertOSGB36toWGS84(pOSGB36:LatLon):LatLon {
		...
		var txFromOSGB36:DatumTransform = {
			tx: -txToOSGB36.tx, ty:-txToOSGB36.ty, tz:-txToOSGB36.tz,
			rx: -txToOSGB36.rx, ry:-txToOSGB36.ry, rz:-txToOSGB36.rz,
			s:-txToOSGB36.s
		};
	 ...
	 */
	public static function convertWGS84toOSGB36(pWGS84) {
	  var eWGS84 = CoordTransform.ellipse.WGS84;
	  var eAiry1830 = CoordTransform.ellipse.Airy1830;
	  var txToOSGB36 = CoordTransform.datumTransform.toOSGB36;
	 ...
	 */
	public static function convertEllipsoid(point:LatLon, e1:EllipseParameter, t:DatumTransform, e2:EllipseParameter):LatLon {
		...
		var rx = Util.toRadians(t.rx / 3600);  // normalise seconds to radians
		var ry = Util.toRadians(t.ry / 3600);
		var rz = Util.toRadians(t.rz / 3600);
		...
		var phi = Math.atan2(z2, p * (1 - eSq)), phiP = Const.TwoPi;
		
		
		while (Math.abs(phi - phiP) > precision) {
			var sPhi 	= Math.sin(phi);
			nu 			= a / Math.sqrt(1.0 - eSq * sPhi * sPhi);
			phiP 		= phi;
			phi 		= Math.atan2(z2 + eSq * nu * sPhi, p);
		}

		var lambda 	= Math.atan2(y2, x2);
		var H 		= p / Math.cos(phi) - nu;

		return new LatLon(Util.toDegrees(phi), Util.toDegrees(lambda), H);
	}
}